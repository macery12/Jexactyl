...
import { LayoutDashboard, Settings, LifeBuoy, ShoppingCart, ReceiptText, Activity, KeyRound } from 'lucide-react';
import { route, type RouteDef } from './registry';

const DashboardPage = lazy(() => import('@/pages/dashboard/DashboardPage'));
const AccountPage = lazy(() => import('@/pages/account/settings/AccountPage'));
const CredentialsPage = lazy(() => import('@/pages/account/credentials/CredentialsPage'));
const OrdersPage = lazy(() => import('@/pages/account/billing/orders/OrdersPage'));
...
export const accountRoutes: RouteDef[] = [
    ...
    route('billing/orders', { name: 'Orders', icon: ReceiptText, element: OrdersPage, condition: f => f.billing.enabled }),
    route('activity', { name: 'Activity', icon: Activity, element: ActivityPage }),
    route('credentials', { name: 'Credentials', icon: KeyRound, element: CredentialsPage }),
